...
class OnboardingActivity : FragmentActivity(), ViewPager.OnPageChangeListener{
    ...
    val BEHAVIOR_RESUME_ONLY_CURRENT_FRAGMENT = 1
    ...
    override fun onPageSelected(position: Int) {
        binding.lytButtons.visibility = if (position + 1 == pagerAdapter.count) View.GONE else View.VISIBLE
    //    binding.introBtnSkip.visibility = binding.introBtnNext.visibility
    ...
    private fun initOnboarding(){
     ...
     //   binding.introBtnSkip.setOnClickListener { skipCallback?.invoke() }
      //  skipText?.let { binding.introBtnSkip.text = it }
        binding.introBtnNext.setOnClickListener { binding.viewPagerContainer.currentItem.let {  binding.viewPagerContainer.currentItem = it+1 } }
    }

    private inner class OnBoardingAdapter(fm: FragmentManager) : FragmentStatePagerAdapter(fm, BEHAVIOR_RESUME_ONLY_CURRENT_FRAGMENT) {
        ...
        binding.indicatorContainer.let {
            if (fragmentList.size > 1)
                for (i in 0 until fragmentList.size) {
                    val indicator = LayoutInflater.from(this).inflate(R.layout.view_indicator,  binding.indicatorContainer, false)
